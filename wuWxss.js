...
function doWxss(dir, cb, mainDir, nowDir) {
    ...
    wu.scanDirByExt(dir, ".html", files => {
        ...
        wu.get(frameFile, code => {
            ...
            // todo add __globalThis
            let env = ""
            if (code.indexOf("__globalThis")){
                env = code.slice(0, code.lastIndexOf('__globalThis.__wcc_version__'))
            }
            // todo add
            let window = {
                screen: {
            ...
            // todo add __globalThis.__wcc_version__
            let scriptCode_index = scriptCode.lastIndexOf('window.__wcc_version__') === -1 ? scriptCode.lastIndexOf('__globalThis.__wcc_version__') : scriptCode.lastIndexOf('window.__wcc_version__')
            scriptCode = scriptCode.slice(scriptCode_index);
