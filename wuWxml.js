...
function doFrame(name, cb, order, mainDir) {
    ...
    wu.get(name, code => {
        getZ(code, z => {
            var rD = {}, rE = {}, rF = {}, requireInfo = {}, x, vm, vmCode;
            if (code.match(/if\(path&&e_\[path\]\)\{/g).length>1){
                var env_code=`window= {"screen":{"width":720,"height":1028,"orientation":{"type":"vertical"}}};
...
\tcreateTextNode: function(){}
                ...
                vmCode = env_code + code;
                glb = {};
                rD = {}, rE = {}, rF = {}, requireInfo = {}, x, vm = new VM({
                sandbox: {
                    __GWX_GLOBAL__: glb
                    }
                });
                ...
                rF = glb.modules
                x = []
                var matches = code.match(/var x=\[(.+)\];/g);
                for (let matche in matches){
                    x.push(matches[0].match(/\'(.+)\'/)[1])
                }
                json = "{}"
            }
            else{
                const before = "\nvar nv_require=function(){var nnm=";
                // if(path&&e_[path]){ mxbc有好几个
                code = code.slice(code.lastIndexOf(before) + before.length,
